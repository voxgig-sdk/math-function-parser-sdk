...
class TokenizeEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = MathFunctionParserSDK.test(seed, nil)
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MATH_FUNCTION_PARSER_TEST_TOKENIZE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    tokenize_ref01_data = nil
    if tokenize_ref01_data_raw.length > 0
...
def tokenize_basic_setup(extra)
  ...
  entid_env_raw = ENV["MATH_FUNCTION_PARSER_TEST_TOKENIZE_ENTID"]
