...
class TokenizeDirectTest < Minitest::Test
  def test_direct_list_tokenize
    setup = tokenize_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-tokenize", setup[:live] ? "live" : "unit")
    if _should_skip
...
def tokenize_direct_setup(mockres)
