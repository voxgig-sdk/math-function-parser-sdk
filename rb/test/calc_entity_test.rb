...
class CalcEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Calc(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Calc(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MATH_FUNCTION_PARSER_TEST_CALC_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def calc_basic_setup(extra)
  ...
  entid_env_raw = ENV["MATH_FUNCTION_PARSER_TEST_CALC_ENTID"]
