...
class CalcEntity
  ...
  def make
    opts = @_entopts.dup
    CalcEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = MathFunctionParserHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Calc, Hash] the current Calc data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = MathFunctionParserHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Calc fields)
  def match_get
  ...
  #
  # @param reqmatch [CalcListMatch, Hash, nil] match filter (any subset of
  #   Calc fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Calc>, Array] the matching Calc items; raises MathFunctionParserError on failure
