...
module MathFunctionParserUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = MathFunctionParserResponse.new({})
    ...
    if fetch_err
      ...
      response = MathFunctionParserResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = MathFunctionParserResponse.new(fetched)
    else
