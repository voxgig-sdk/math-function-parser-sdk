...
class MathFunctionParserSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = MathFunctionParserHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = MathFunctionParserHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, MathFunctionParserFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    MathFunctionParserUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    params = MathFunctionParserHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = MathFunctionParserHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => MathFunctionParserError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = MathFunctionParserHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = MathFunctionParserError.new(
        "graphql_error", "MathFunctionParserSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = MathFunctionParserSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
