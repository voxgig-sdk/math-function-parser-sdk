// Command MathFunctionParser-mcp is an MCP server exposing the MathFunctionParser
// SDK as MCP tools.
//
// Two tools are registered:
//
//	MathFunctionParser_list  — list records of an entity
//	MathFunctionParser_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: MATH_FUNCTION_PARSER_APIKEY carries the API key and
	// MATH_FUNCTION_PARSER_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MATH_FUNCTION_PARSER_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MATH_FUNCTION_PARSER_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMathFunctionParserSDK(opts)
