# math-function-parser-mcp

[MCP](https://modelcontextprotocol.io) server exposing the MathFunctionParser SDK as
two agent tools — `math-function-parser_list` and `math-function-parser_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/math-function-parser-mcp)
make build
...
claude mcp add --scope user math-function-parser \
  -- /absolute/path/to/math-function-parser-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./math-function-parser-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// math-function-parser_list: first page of records
{ "entity": "calc" }
{ "entity": "calc", "query": { } }

// math-function-parser_load: one record by id
{ "entity": "resolve", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/math-function-parser-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user math-function-parser \
     -- "$PWD"/dist/*/math-function-parser-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `math-function-parser_list` and `math-function-parser_load` tools now appear
   in new sessions. Ask the agent to *"list calc using math-function-parser"*
   and it calls `math-function-parser_list` with `{"entity":"calc"}`.
...
### Call the `math-function-parser_list` tool
...
### Call the `math-function-parser_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `math-function-parser_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `math-function-parser_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MATH_FUNCTION_PARSER_APIKEY` | API key sent with every request. |
| `MATH_FUNCTION_PARSER_BASE` | Optional override of the API base URL. |
...
```sh
./math-function-parser-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"math-function-parser_load","arguments":{"entity":"resolve","query":{"id":1}}}}'
```
