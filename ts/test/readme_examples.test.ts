...
//     (\`new MathFunctionParserSDK(...)\` / \`MathFunctionParserSDK.test(...)\` become
//     \`MathFunctionParserSDK.test({ entity: ... })\`), transpiled to JS, and run
...
// rewrite real client construction (\`new MathFunctionParserSDK(...)\` and any
// \`MathFunctionParserSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+MathFunctionParserSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/MathFunctionParserSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
