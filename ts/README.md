# MathFunctionParser TypeScript SDK

The TypeScript SDK for the MathFunctionParser API. Provides a type-safe, entity-oriented interface with full async/await support.
...
```ts
const client = new MathFunctionParserSDK({ apikey: '...' })
...
```ts
new MathFunctionParserSDK(options?: {
  apikey?: string
  base?: string
  prefix?: string
  suffix?: string
...
| `tester(testopts?, sdkopts?)` | `MathFunctionParserSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `MathFunctionParserSDK.test(testopts?, sdkopts?)` | `MathFunctionParserSDK` | Create a test-mode client. |
...
├── src/
│   ├── MathFunctionParserSDK.ts        # Main SDK class
