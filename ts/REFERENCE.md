...
```ts
const results = await client.Calc().list({ expression: "example" })
```
...
```ts
const result = await client.Resolve().load({ expression: 'expression' })
```
...
```ts
const results = await client.Tokenize().list({ expression: "example" })
```
