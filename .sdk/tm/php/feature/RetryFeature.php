...
// MathFunctionParser SDK retry feature
...
class MathFunctionParserRetryFeature extends MathFunctionParserBaseFeature
{
    ...
    public function init(MathFunctionParserContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (MathFunctionParserContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(MathFunctionParserContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(MathFunctionParserContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
