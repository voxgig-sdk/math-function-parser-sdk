...
// MathFunctionParser SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = MathFunctionParserOmni::makeStructRunner(
    ...
    {
        return MathFunctionParserOmni::objmodel(MathFunctionParserOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return MathFunctionParserOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return MathFunctionParserOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(MathFunctionParserOmni::structfix($now, Struct::undef()))
                        !== json_encode(MathFunctionParserOmni::structfix($val, Struct::undef()))) {
