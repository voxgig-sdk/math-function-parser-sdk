...
// MathFunctionParser SDK feature test
//
...
class FtCtrl extends MathFunctionParserControl
{
...
class FtClient
{
    ...
    private MathFunctionParserUtility $utility;

    public function __construct(MathFunctionParserUtility $utility, array $options)
    {
    ...
    public function get_utility(): MathFunctionParserUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MathFunctionParserContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public MathFunctionParserContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MathFunctionParserContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MathFunctionParserConfig::make_feature($name);
    ...
    private static function buildUrl(MathFunctionParserSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MathFunctionParserError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MathFunctionParserResponse($fetched);
        }
    ...
    private function fail(MathFunctionParserContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MathFunctionParserContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
