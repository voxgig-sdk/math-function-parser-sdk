# MathFunctionParser Lua SDK

The Lua SDK for the MathFunctionParser API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Calc(nil):list(nil, nil)
...
local result, err = client:MathFunctionParser(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── math-function-parser_sdk.lua    -- Main SDK module
...
The main module (`math-function-parser_sdk`) exports the SDK constructor
