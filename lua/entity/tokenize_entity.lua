...
local TokenizeEntity = {}
TokenizeEntity.__index = TokenizeEntity


function TokenizeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TokenizeEntity)
  self._name = "tokenize"
  self._client = client
...
function TokenizeEntity:get_name()
  return self._name
end


function TokenizeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TokenizeEntity.new(self._client, opts)
end
...
function TokenizeEntity:mark_deleted()
  self._deleted = true
end


function TokenizeEntity:deleted()
  return true == self._deleted
end


function TokenizeEntity:data_set(args)
  if args ~= nil then
...
function TokenizeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TokenizeEntity:match_set(args)
  if args ~= nil then
...
function TokenizeEntity:match_get()
...
function TokenizeEntity:stream(action, args, callopts)
...
function TokenizeEntity:_run_op(ctx, post_done)
  local utility = self._utility
