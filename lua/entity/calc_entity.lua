...
local CalcEntity = {}
CalcEntity.__index = CalcEntity


function CalcEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CalcEntity)
  self._name = "calc"
  self._client = client
...
function CalcEntity:get_name()
  return self._name
end


function CalcEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CalcEntity.new(self._client, opts)
end
...
function CalcEntity:mark_deleted()
  self._deleted = true
end


function CalcEntity:deleted()
  return true == self._deleted
end


function CalcEntity:data_set(args)
  if args ~= nil then
...
function CalcEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CalcEntity:match_set(args)
  if args ~= nil then
...
function CalcEntity:stream(action, args, callopts)
...
function CalcEntity:_run_op(ctx, post_done)
  local utility = self._utility
