# MathFunctionParser Lua SDK Reference

Complete API reference for the MathFunctionParser Lua SDK.
...
```lua
local results, err = client:Calc(nil):list(nil, nil)
```
...
```lua
local results, err = client:Tokenize(nil):list(nil, nil)
```
