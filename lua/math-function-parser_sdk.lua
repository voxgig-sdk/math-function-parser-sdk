...
require("math-function-parser_types")
...
local MathFunctionParserSDK = {}
MathFunctionParserSDK.__index = MathFunctionParserSDK
...
MathFunctionParserSDK._make_feature = _make_feature


function MathFunctionParserSDK.new(options)
  local self = setmetatable({}, MathFunctionParserSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function MathFunctionParserSDK:options_map()
...
function MathFunctionParserSDK:get_utility()
  return Utility.copy(self._utility)
end


function MathFunctionParserSDK:get_root_ctx()
  return self._rootctx
end


function MathFunctionParserSDK:prepare(fetchargs)
...
function MathFunctionParserSDK:direct(fetchargs)
...
function MathFunctionParserSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function MathFunctionParserSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "MathFunctionParserSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function MathFunctionParserSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function MathFunctionParserSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "MathFunctionParserSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function MathFunctionParserSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = MathFunctionParserSDK.new(sdkopts)
  sdk.mode = "test"
