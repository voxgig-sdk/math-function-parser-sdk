...
type TokenizeEntity struct {
	name    string
	client  *core.MathFunctionParserSDK
...
func NewTokenizeEntity(client *core.MathFunctionParserSDK, entopts map[string]any) *TokenizeEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
func (e *TokenizeEntity) GetName() string { return e.name }
...
func (e *TokenizeEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *TokenizeEntity) Deleted() bool {
	return e.deleted
}


func (e *TokenizeEntity) Make() core.Entity {
	opts := map[string]any{}
	for k, v := range e.entopts {
		opts[k] = v
	}
	return NewTokenizeEntity(e.client, opts)
}

func (e *TokenizeEntity) Data(args ...any) any {
...
func (e *TokenizeEntity) Match(args ...any) any {
	if len(args) > 0 && args[0] != nil {
...
// argument it returns the current data as an Tokenize; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *TokenizeEntity) DataTyped(data ...Tokenize) Tokenize {
	if len(data) > 0 {
		return typedFrom[Tokenize](e.Data(asMap(data[0])))
	}
	return typedFrom[Tokenize](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through Tokenize (all fields
// optional at the wire level).
func (e *TokenizeEntity) MatchTyped(match ...Tokenize) Tokenize {
	if len(match) > 0 {
		return typedFrom[Tokenize](e.Match(asMap(match[0])))
	}
	return typedFrom[Tokenize](e.Match())
}
...
func (e *TokenizeEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
func (e *TokenizeEntity) Load(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("load", e.name)
}



func (e *TokenizeEntity) List(reqmatch map[string]any, ctrl map[string]any) (any, error) {
	utility := e.utility
...
// TokenizeListMatch and returns []Tokenize. It delegates to the untyped
// List (identical runtime) and converts at the typed boundary.
func (e *TokenizeEntity) ListTyped(reqmatch TokenizeListMatch, ctrl map[string]any) ([]Tokenize, error) {
	res, err := e.List(asMap(reqmatch), ctrl)
	if err != nil {
		return nil, err
	}
	return typedSliceFrom[Tokenize](res), nil
}



func (e *TokenizeEntity) Create(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("create", e.name)
}


func (e *TokenizeEntity) Update(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("update", e.name)
}


func (e *TokenizeEntity) Remove(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("remove", e.name)
}


func (e *TokenizeEntity) runOp(ctx *core.Context, postDone func()) (any, error) {
	utility := e.utility
