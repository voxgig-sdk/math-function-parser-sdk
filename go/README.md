...
## Install
```bash
go get github.com/voxgig-sdk/math-function-parser-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/math-function-parser-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/math-function-parser-sdk/go=../math-function-parser-sdk/go
```
...
calc, err := client.Calc(nil).Load(
    map[string]any{"id": "test01"}, nil,
)
if err != nil {
    panic(err)
}
fmt.Println(calc) // the loaded mock data
```
...
### NewMathFunctionParserSDK

```go
func NewMathFunctionParserSDK(options map[string]any) *MathFunctionParserSDK
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *MathFunctionParserSDK
```

Creates a test-mode client with mock transport. Both arguments may be `nil`.

### MathFunctionParserSDK methods
...
### Entity interface (MathFunctionParserEntity)

All entities implement the `MathFunctionParserEntity` interface.
...
slice):
    ...
    calc, err := client.Calc(nil).Load(map[string]any{"id": "example_id"}, nil)
    if err != nil { /* handle */ }
...
Create an instance: `calc := client.Calc(nil)`
...
fmt.Println(calcs) // the array of records
```
...
```
github.com/voxgig-sdk/math-function-parser-sdk/go/
├── math-function-parser.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/math-function-parser-sdk/go`) re-exports everything needed
...
calc.Load(map[string]any{"id": "example_id"}, nil)
