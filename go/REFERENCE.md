# MathFunctionParser Golang SDK Reference

Complete API reference for the MathFunctionParser Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *MathFunctionParserSDK`
...
#### `Resolve(data map[string]any) MathFunctionParserEntity`

Create a new `Resolve` entity instance. Pass `nil` for no initial data.

#### `Tokenize(data map[string]any) MathFunctionParserEntity`
...
```go
result, err := client.Resolve(nil).Load(map[string]any{"expression": "expression"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
