...
import (
	...
	omni "github.com/voxgig-sdk/math-function-parser-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.MathFunctionParserError); is {
				out := map[string]any{
					"name":    "MathFunctionParserError",
...
func omniCtx(arg any, client *sdk.MathFunctionParserSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.MathFunctionParserSDK); is {
			client = live
