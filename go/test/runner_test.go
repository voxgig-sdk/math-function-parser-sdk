...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("MATHFUNCTIONPARSER_TEST_LIVE") == "TRUE" ||
		os.Getenv("MATHFUNCTIONPARSER_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("MATHFUNCTIONPARSER_TEST_EXPLAIN"); explain != "" {
