...
func TestResolveEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set MATHFUNCTIONPARSER_TEST_RESOLVE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func resolveBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MATHFUNCTIONPARSER_TEST_RESOLVE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MATHFUNCTIONPARSER_TEST_RESOLVE_ENTID"])
