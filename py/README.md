# MathFunctionParser Python SDK

The Python SDK for the MathFunctionParser API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Calc(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.MathFunctionParser(None).load(
    {"id": "test01"}, None
)
...
```python
client = MathFunctionParserSDK.test(testopts, sdkopts)
```
...
```
py/
├── mathfunctionparser_sdk.py         -- Main SDK module
...
The main module (`mathfunctionparser_sdk`) exports the SDK class.
