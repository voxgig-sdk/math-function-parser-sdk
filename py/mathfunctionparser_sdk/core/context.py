...
from mathfunctionparser_sdk.core.helpers import get_ctx_prop, to_map
...
class MathFunctionParserContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = MathFunctionParserControl()
        ...
        if isinstance(sp, MathFunctionParserSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, MathFunctionParserResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, MathFunctionParserResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return MathFunctionParserOperation({})
        ...
        op = MathFunctionParserOperation({
            "entity": entname,
            "name": opname,
            "input": inpt,
            "points": points,
    ...
    def make_error(self, code, msg):
        return MathFunctionParserError(code, msg, self)
