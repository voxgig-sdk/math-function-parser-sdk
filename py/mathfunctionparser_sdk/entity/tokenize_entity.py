...
from mathfunctionparser_sdk.mathfunctionparser_types import (
    Tokenize,
    TokenizeListMatch,
)
...
class TokenizeEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return TokenizeEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Tokenize]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Tokenize().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
