# MathFunctionParser Python SDK Reference

Complete API reference for the MathFunctionParser Python SDK.
...
#### `MathFunctionParserSDK.test(testopts=None, sdkopts=None)`
...
```python
results = client.Calc().list({"expression": "example"})
for calc in results:
    print(calc)
```
...
Create a new `CalcEntity` instance with the same options.
...
```python
result = client.Resolve().load({"expression": "expression"})
```
...
```python
results = client.Tokenize().list({"expression": "example"})
