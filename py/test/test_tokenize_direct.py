...
class TestTokenizeDirect:

    def test_should_direct_list_tokenize(self):
        setup = _tokenize_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-tokenize", "live" if setup["live"] else "unit")
        if _skip:
...
def _tokenize_direct_setup(mockres):
