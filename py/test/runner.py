# MathFunctionParser SDK test runner
...
class MathFunctionParserTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MathFunctionParserTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MathFunctionParserTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MathFunctionParserTestRunner.getenv("MATH_FUNCTION_PARSER_TEST_LIVE")
        override = MathFunctionParserTestRunner.getenv("MATH_FUNCTION_PARSER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MathFunctionParserTestRunner.getenv(key)
        ...
        explain = MathFunctionParserTestRunner.getenv("MATH_FUNCTION_PARSER_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if MathFunctionParserTestRunner._test_control is not None:
            return MathFunctionParserTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MathFunctionParserTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MathFunctionParserTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MathFunctionParserTestRunner.load_test_control()
...
def load_env_local():
    MathFunctionParserTestRunner.load_env_local()


def env_override(m):
    return MathFunctionParserTestRunner.env_override(m)


def entity_data(v):
    return MathFunctionParserTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MathFunctionParserTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MathFunctionParserTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MathFunctionParserTestRunner.load_test_control()


def live_client_options():
    return MathFunctionParserTestRunner.live_client_options()


def live_delay_ms():
    return MathFunctionParserTestRunner.live_delay_ms()
