...
class TestCalcDirect:
    ...
    def test_should_direct_list_calc(self):
        setup = _calc_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-calc", "live" if setup["live"] else "unit")
        if _skip:
...
def _calc_direct_setup(mockres):
