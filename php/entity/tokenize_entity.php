...
// MathFunctionParser SDK Tokenize entity
...
class TokenizeEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new TokenizeEntity($this->_client, $opts);
    }

    /**
     * @param Tokenize|array $args Tokenize data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = MathFunctionParserHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Tokenize|array The current Tokenize data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Tokenize fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = MathFunctionParserHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Tokenize fields).
     */
     ...
     *
     * @param TokenizeListMatch|array|null $reqmatch Match filter (any subset
     *   of Tokenize fields) as an assoc-array; TokenizeListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Tokenize[]|array A list of Tokenize items as assoc-arrays at
     *   the SDK boundary; throws MathFunctionParserError on failure (item-5 convention).
     */
