...
class MathFunctionParserSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = MathFunctionParserHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = MathFunctionParserHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!MathFunctionParserFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, MathFunctionParserFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return MathFunctionParserUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = MathFunctionParserHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = MathFunctionParserHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = MathFunctionParserHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new MathFunctionParserError($op . "_allow",
                "MathFunctionParserSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = MathFunctionParserHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new MathFunctionParserError("graphql_error",
                "MathFunctionParserSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Calc($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_calc === null) {
                $this->_calc = new CalcEntity($this, null);
            }
            return $this->_calc;
        }
        return new CalcEntity($this, $data);
    }
    ...
    public function Tokenize($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_tokenize === null) {
                $this->_tokenize = new TokenizeEntity($this, null);
            }
            return $this->_tokenize;
        }
        return new TokenizeEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new MathFunctionParserSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
