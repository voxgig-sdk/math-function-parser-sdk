...
// MathFunctionParser SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class MathFunctionParserTestFeature extends MathFunctionParserBaseFeature
{
    ...
    public function init(MathFunctionParserContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (MathFunctionParserContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = MathFunctionParserParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (MathFunctionParserContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(MathFunctionParserContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = MathFunctionParserParam::call($ctx, $k);
