...
// MathFunctionParser SDK pipeline test
//
...
{
    private static function utility(): MathFunctionParserUtility
    {
        return new MathFunctionParserUtility();
    }
    ...
    private static function ctx(array $over = []): MathFunctionParserContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new MathFunctionParserContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new MathFunctionParserOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof MathFunctionParserError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new MathFunctionParserOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = MathFunctionParserMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new MathFunctionParserOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = MathFunctionParserMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new MathFunctionParserOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = MathFunctionParserMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new MathFunctionParserSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = MathFunctionParserMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new MathFunctionParserResponse([]),
            'result' => new MathFunctionParserResult([]),
        ]);
        [, $err] = MathFunctionParserMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new MathFunctionParserSpec([]),
            'response' => null,
            'result' => new MathFunctionParserResult([]),
        ]);
        [, $err] = MathFunctionParserMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new MathFunctionParserSpec(['step' => 's']),
            'response' => new MathFunctionParserResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new MathFunctionParserResult([]),
        ]);
        [, $err] = MathFunctionParserMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new MathFunctionParserSpec(['step' => 's']),
            'response' => new MathFunctionParserResponse(self::resp(200, ['v' => 1])),
            'result' => new MathFunctionParserResult([]),
        ]);
        [, $err] = MathFunctionParserMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new MathFunctionParserSpec(['step' => 's']),
            'response' => new MathFunctionParserResponse(self::resp(200, ['v' => 2])),
            'result' => new MathFunctionParserResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        MathFunctionParserMakeResponse::call($ctx);
    ...
    {
        $preset = new MathFunctionParserResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new MathFunctionParserSpec([]),
            'response' => new MathFunctionParserResponse([]),
            'result' => new MathFunctionParserResult([]),
        ...
        [$got, $err] = MathFunctionParserMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new MathFunctionParserResult([])]);
        [, $err] = MathFunctionParserMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new MathFunctionParserSpec([]), 'result' => null]);
        [, $err] = MathFunctionParserMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new MathFunctionParserSpec(['step' => 's']),
            'result' => new MathFunctionParserResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new MathFunctionParserOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = MathFunctionParserMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new MathFunctionParserSpec(['step' => 's']),
            'result' => new MathFunctionParserResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new MathFunctionParserOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = MathFunctionParserMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new MathFunctionParserResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new MathFunctionParserSpec([]),
            'result' => new MathFunctionParserResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = MathFunctionParserMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = MathFunctionParserMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new MathFunctionParserError('boom', 'boom');
        $utility->fetcher = function (MathFunctionParserContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new MathFunctionParserSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = MathFunctionParserMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (MathFunctionParserContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = MathFunctionParserMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(MathFunctionParserResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        MathFunctionParserMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (MathFunctionParserContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new MathFunctionParserSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = MathFunctionParserMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new MathFunctionParserResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new MathFunctionParserSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = MathFunctionParserMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = MathFunctionParserMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = MathFunctionParserMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new MathFunctionParserResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, MathFunctionParserDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new MathFunctionParserResult(['ok' => false])]);
        $this->expectException(MathFunctionParserError::class);
        MathFunctionParserDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new MathFunctionParserResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', MathFunctionParserMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        MathFunctionParserMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new MathFunctionParserBaseFeature();
        $b = new MathFunctionParserBaseFeature();
        MathFunctionParserFeatureAdd::call($ctx, $a);
        MathFunctionParserFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): MathFunctionParserBaseFeature
    {
        $f = new MathFunctionParserBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        MathFunctionParserFeatureAdd::call($ctx, self::named_feature('a'));
        MathFunctionParserFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        MathFunctionParserFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        MathFunctionParserFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        MathFunctionParserFeatureAdd::call($ctx, $replace);
        ...
        MathFunctionParserFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): MathFunctionParserContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new MathFunctionParserSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = MathFunctionParserPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        MathFunctionParserPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new MathFunctionParserResponse(['headers' => null]),
            'result' => new MathFunctionParserResult([]),
        ]);
        MathFunctionParserResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        MathFunctionParserResultBody::call($ctx);
