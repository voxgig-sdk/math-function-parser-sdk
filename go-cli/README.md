...
AQL-driven command-line client **and** interactive REPL for the MathFunctionParser
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/math-function-parser-cli)
make build

# 2. See usage (words, entities, env vars)
./math-function-parser-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/math-function-parser-cli
   ```
...
Run with no arguments to open a REPL (prompt `math-function-parser>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `calc`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/math-function-parser-cli`. |
...
(`./math-function-parser-cli <expr>`) and interactively (the REPL), and expressions compose the
